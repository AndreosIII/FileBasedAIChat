...
def load_json_config(file_path):
    ...
    Args:
        file_path: Путь к JSON файлу, который нужно загрузить.
...
def is_model_valid(model_name, models_config_path=MODELS_CONFIG_PATH):
    """
    Проверяет, существует ли модель с указанным именем в конфигурационном файле моделей.
    
    Args:
        model_name: Имя модели для проверки.
        models_config_path: Путь к файлу конфигурации моделей.
        
    Returns:
        True, если модель существует, иначе False.
    """
    models_config = load_json_config(models_config_path)
    return model_name in models_config

def is_behavior_valid(behavior_name, behavior_config_path=BEHAVIOR_CONFIG_PATH):
    """
    Проверяет, существует ли шаблон поведения с указанным именем в конфигурационном файле шаблонов поведения.
    
    Args:
        behavior_name: Имя шаблона поведения для проверки.
        behavior_config_path: Путь к файлу конфигурации шаблонов поведения.
        
    Returns:
        True, если шаблон поведения существует, иначе False.
    """
    behavior_config = load_json_config(behavior_config_path)
    return behavior_name in behavior_config

def create_default_dialog_file(file_path, default_config_path=DEFAULT_DIALOG_CONFIG_PATH):
    """
    Создает новый файл диалога с использованием стандартных настроек модели и поведения из конфигурационного файла.
    
    Args:
        file_path: Путь, по которому будет создан файл диалога.
        default_config_path: Путь к файлу со стандартной конфигурацией диалога.
...
def check_or_create_dialog_file(file_path, default_config_path=DEFAULT_DIALOG_CONFIG_PATH):
...
def parse_model_from_dialog(dialog_content):
    """
...
def parse_behavior_from_dialog(dialog_content):
    ...
    if behavior_match:
        ...
        if is_behavior_valid(behavior_name):
            behavior_config = load_json_config(BEHAVIOR_CONFIG_PATH)
            behavior_data = behavior_config.get(behavior_name)
            if behavior_data:
...
def parse_dialog_file(file_path):
    """Основная функция для парсинга файла диалога. Возвращает информацию о модели, поведении и высказываниях участников диалога.

    Args:
        file_path: Путь к файлу диалога, который нужно распарсить.
...
def append_ai_response_to_dialog(file_path, response_generator):
